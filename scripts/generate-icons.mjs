...
const PREFIX_RE = new RegExp(
  `\\b(${Object.keys(PACKAGE_BY_PREFIX).join("|")}):[a-z0-9-]+`,
  "g",
);
...
async function main() {
  ...
  for (const c of collections)
    registerLines.push(`addCollection(${JSON.stringify(c)});`);
  registerLines.push("");
  fs.writeFileSync(REGISTER_FILE, registerLines.join("\n"), "utf-8");
  const regTotal = collections.reduce(
    (acc, c) => acc + Object.keys(c.icons).length,
    0,
  );
