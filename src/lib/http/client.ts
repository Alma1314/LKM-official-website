  ...
  if (!_instance) {
    ...
    _instance.interceptors.response.use(
      ...
      async (error: unknown) => {
        ...
        try {
          ...
          if (refreshToken) {
            const res = await _instance!.post(
              "/api/v1/auth/refresh",
            ...
            const apiResp = res.data as {
              data?: { access_token: string; refresh_token: string };
            };
            if (apiResp?.data?.access_token) {
              getAdapter().setTokens(
                apiResp.data.access_token,
                apiResp.data.refresh_token,
              );
    ...
    _instance.interceptors.response.use(
      ...
      (error: unknown) => {
        ...
        if (!error.response) {
          return Promise.reject(
            new AppError(ErrorCode.NETWORK_ERROR, t("messages.networkError")),
          );
        }
        ...
        return Promise.reject(
          new AppError(
            code,
            t("messages.requestFailed", { status }) +
              (detail ? `：${detail}` : ""),
            error,
          ),
        );
  ...
  try {
    ...
    return err(
      new AppError(
        ...
        t("messages.unknownNetworkError"),
        e,
      ),
    );
  }
