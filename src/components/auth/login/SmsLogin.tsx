...
export function SmsLogin({ onLogin, identifiedAccount }: Props) {
  ...
  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    if (!code.trim()) { setCodeError('请输入验证码'); return; }
    if (attempts >= 3) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {codeSent && (
        <p className="text-xs text-success text-center">验证码已发送（模拟码：000000）</p>
      )}
      <div>
        ...
        <input
          ...
          value={code}
          onInput={(e) => { setCode(e.currentTarget.value); setCodeError(''); }}
