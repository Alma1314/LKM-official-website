      ...
      <div className="relative min-h-[calc(100vh-12rem)] flex items-center justify-center px-4">
        ...
        <div className="relative w-full max-w-md text-center">
          <div className="rounded-2xl bg-base-100 shadow-2xl border border-base-300 p-6 sm:p-8">
            ...
            <p className="text-neutral text-sm">欢迎加入理科迷，<span className="font-semibold">{state.user?.username}</span></p>
      ...
      <div className="relative w-full max-w-md">
        <div className="rounded-2xl bg-base-100 shadow-2xl border border-base-300 p-6 sm:p-8">
          ...
          <div className="tabs tabs-bordered mb-6">
            {([{ key: 'normal' as RegType, label: '普通账户' }, { key: 'local' as RegType, label: '本地账户' }, { key: 'github' as RegType, label: 'GitHub' }]).map((tab) => (
              <a key={tab.key} className={`tab tab-bordered${regType === tab.key ? ' tab-active' : ''}`} onClick={(e) => { e.preventDefault(); setRegType(tab.key); }}>{tab.label}</a>
            ))}
          </div>

          {regType === 'local' && <LocalRegister onRegister={handleRegister} />}
          {regType === 'normal' && <NormalRegister onRegister={handleRegister} onComplete={(withGuide) => setShowGuide(withGuide)} />}
          {regType === 'github' && (
            <div className="space-y-4 text-center">
              <p className="text-sm text-neutral">GitHub OAuth 注册（模拟）</p>
              <button type="button" className="btn btn-outline w-full gap-2" onClick={() => { register('normal', { username: 'github_user_' + Date.now().toString(36), password: '123456', email: 'github@likemi.com' }); }}>
          ...
          <p className="text-center text-[13px] text-neutral mt-5">
            已有账号？<a href="/login" className="text-primary font-semibold hover:underline">立即登录</a>
          </p>
        </div>
      </div>
    </div>
