    ...
    <div class="relative min-h-[calc(100vh-12rem)] px-4 py-8">
      <div class="max-w-2xl mx-auto space-y-6">
        <div class="text-center mb-8">
          ...
          <p class="text-sm text-[var(--text-muted)]">管理你的登录方式和安全设置</p>
        </div>

        <div v-if="message" class="alert alert-success text-sm">{{ message }}</div>

        <!-- Account info -->
        <div class="rounded-2xl bg-[var(--card-bg)] shadow-xl border border-[var(--surface-3)] p-6 space-y-4">
          <h3 class="text-lg font-semibold">账户信息</h3>
          <div class="grid grid-cols-2 gap-3 text-sm">
            <div>
              <span class="text-[var(--text-muted)]">用户名：</span>
              <span class="font-medium">{{ state.user?.username }}</span>
            </div>
            <div>
              <span class="text-[var(--text-muted)]">等级：</span>
              <span class="badge badge-sm" :class="levelBadgeClass">{{ levelLabel }}</span>
            </div>
            <div>
              <span class="text-[var(--text-muted)]">邮箱：</span>
              <span class="font-medium">{{ state.user?.email || '未绑定' }}</span>
            </div>
            <div>
              <span class="text-[var(--text-muted)]">手机号：</span>
        ...
        <!-- Bindings -->
        <div v-if="state.user" class="rounded-2xl bg-[var(--card-bg)] shadow-xl border border-[var(--surface-3)] p-6">
        ...
        <div class="flex gap-3 justify-between">
          ...
          <button type="button" class="btn btn-outline btn-sm text-[var(--error)]" @click="logout">退出登录</button>
        </div>
      </div>
    </div>
  </ProtectedRoute>
