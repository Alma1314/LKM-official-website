...
export function TwoFactorSetup({ user, onUpdate }: Props) {
  ...
  function handleEnable() { setShow(true); setStep('scan'); setCode(''); setError(''); }

  function handleVerify() {
    if (code !== '000000' && code !== '123456') { setError('验证码错误（模拟码：000000）'); return; }
    setError('');
    setStep('codes');
  }
  ...
  function handleDisable() {
    ...
    if (user.level === 'admin') { updated.level = 'normal'; alert('关闭 2FA 后，管理员账户已自动降级为普通账户'); }
    onUpdate(updated);
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between p-3 bg-base-200 rounded-lg">
        <div>
          ...
          <span className={`badge badge-xs ml-2 ${user.has2FA ? 'badge-success' : 'badge-ghost'}`}>{user.has2FA ? '已开启' : '未开启'}</span>
        </div>
        {user.has2FA ? (
          ...
          <button type="button" className="btn btn-ghost btn-xs" onClick={handleEnable}>开启</button>
        )}
      </div>
            ...
            {step === 'scan' ? (
              <div className="space-y-4">
                ...
                <div className="bg-base-200 rounded-xl p-6 mx-auto w-40 h-40 flex items-center justify-center">
                  <div className="text-center"><div className="text-4xl mb-2">📱</div><p className="text-xs text-neutral">模拟二维码</p></div>
                </div>
                <div>
                  <label className="label pb-1"><span className="label-text">输入验证码确认</span></label>
                ...
                <button type="button" className="btn btn-primary btn-sm w-full" onClick={handleVerify}>验证</button>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                <div className="bg-base-200 rounded-xl p-3 font-mono text-xs space-y-1">
                  {DUMMY_RECOVERY_CODES.map((c) => <div className="select-all" key={c}>{c}</div>)}
                </div>
                ...
                <button type="button" className="btn btn-primary btn-sm w-full" disabled={!savedCodes} onClick={handleConfirm}>完成设置</button>
              </div>
            )}
            <button type="button" className="btn btn-ghost btn-xs w-full mt-2" onClick={() => setShow(false)}>取消</button>
          </div>
        </div>
      )}
    </div>
