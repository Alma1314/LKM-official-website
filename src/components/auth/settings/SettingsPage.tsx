        ...
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div className="rounded-2xl bg-base-100 shadow-xl border border-base-300 p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><span className="text-neutral">用户名：</span><span className="font-medium">{state.user?.username}</span></div>
              <div>
                <span className="text-neutral">等级：</span>
                <span className={`badge badge-sm ${state.user?.level === 'admin' ? 'badge-error' : state.user?.level === 'normal' ? 'badge-primary' : 'badge-ghost'}`}>
                  {state.user?.level === 'admin' ? '管理员' : state.user?.level === 'normal' ? '普通账户' : '本地账户'}
                </span>
              </div>
              <div><span className="text-neutral">邮箱：</span><span className="font-medium">{state.user?.email || '未绑定'}</span></div>
              <div><span className="text-neutral">手机号：</span><span className="font-medium">{state.user?.phone || '未绑定'}</span></div>
            </div>
          ...
          <div className="flex gap-3 justify-between">
            <a href="/account/recovery" className="btn btn-ghost btn-sm">密码找回</a>
            <button type="button" className="btn btn-outline btn-sm text-error" onClick={logout}>退出登录</button>
          </div>
        </div>
      </div>
    </ProtectedRoute>
