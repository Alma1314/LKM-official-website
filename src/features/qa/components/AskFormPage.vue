  ...
  <div class="max-w-lg mx-auto space-y-6">
    ...
    <div class="flex gap-2">
      <button
        ...
        :class="activeTab === tab.key ? 'bg-primary text-on-primary' : 'bg-surface-3 text-text-muted'"
    ...
    <div v-else class="space-y-3">
      <div>
        <label class="text-sm font-medium text-deep-text">省份</label
        ><input
          v-model="volunteerData.province"
          class="w-full mt-1 px-3 py-2 rounded-lg border border-surface-3 bg-card-bg text-sm focus:border-primary outline-none"
          placeholder="如：广东"
        />
      </div>
      <div>
        <label class="text-sm font-medium text-deep-text">分数/排名</label
        ><input
          v-model="volunteerData.score"
          class="w-full mt-1 px-3 py-2 rounded-lg border border-surface-3 bg-card-bg text-sm focus:border-primary outline-none"
          placeholder="如：640分 / 省排 12000"
        />
      </div>
      <div>
        <label class="text-sm font-medium text-deep-text">选科</label>
        <div class="flex flex-wrap gap-2 mt-1">
          <button
            v-for="s in ['物理', '历史', '化学', '生物', '政治', '地理']"
            :key="s"
            class="px-3 py-1 rounded-full text-xs border transition-colors"
            :class="
              volunteerData.subjects.includes(s)
      ...
      <div>
        <label class="text-sm font-medium text-deep-text">兴趣方向</label
        ><input
          v-model="volunteerData.interests"
          class="w-full mt-1 px-3 py-2 rounded-lg border border-surface-3 bg-card-bg text-sm focus:border-primary outline-none"
          placeholder="如：计算机、医学、师范"
        />
      </div>
      <div>
        <label class="text-sm font-medium text-deep-text">补充说明（选填）</label
        ><textarea
          v-model="volunteerData.note"
          rows="2"
          class="w-full mt-1 px-3 py-2 rounded-lg border border-surface-3 bg-card-bg text-sm focus:border-primary outline-none resize-none"
          placeholder="如：家庭情况、经济考量等"
        ></textarea>
      </div>
      <button
        class="btn-primary w-full py-2.5 rounded-lg text-sm font-semibold"
        :disabled="!volunteerData.province"
