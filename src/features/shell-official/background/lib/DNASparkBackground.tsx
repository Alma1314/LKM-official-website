...
import { BackgroundCanvas } from '~/features/shell-official/background/BackgroundCanvas';
import type { BackgroundFrame } from '~/features/shell-official/background/useBackgroundCanvas';
import { useColorMode } from '~/features/shell-official/background/useColorMode';
...
export default function DNASparkBackground({
  ...
  color: _color,
  constfill: _constfill,
  text: _text,
}: DNASparkBackgroundProps) {
  ...
  const initBasePairs = (width: number, height: number) => {
  ...
  const draw = useCallback(
    (frame: BackgroundFrame) => {
      ...
      if (width !== lastSizeRef.current.width || height !== lastSizeRef.current.height) {
        ...
        basePairsRef.current = initBasePairs(width, height);
        initialAnimationCompleteRef.current = false;
        isShatteringRef.current = false;
        shatterProgressRef.current = 0;
        ripplesRef.current = [];
      ...
      basePairsRef.current.forEach((basePair, index) => {
        ...
        if (!initialAnimationCompleteRef.current) {
          ...
          const shatterProgress = shatterProgressRef.current / 1;
