...
<div class="max-w-5xl mx-auto px-4 py-8">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
    {#each plugins as plugin (plugin.id)}
      <div class="card-base p-6 cursor-pointer hover:border-primary/30 transition-colors" role="button" tabindex="0" onclick={() => { selectedPlugin = plugin.id; }} onkeydown={(e) => e.key === 'Enter' && (selectedPlugin = plugin.id)}>
