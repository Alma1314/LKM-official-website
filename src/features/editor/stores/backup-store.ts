import { ok, err, AppError, ErrorCode } from '~/core/errors';
...
export async function saveBackup(docId: string, data: BackupData): Promise<Result<void>> {
  try {
    ...
    if (!db) return err(new AppError(ErrorCode.DB_OPEN_FAILED, 'IndexedDB 不可用'));
    ...
    return err(new AppError(ErrorCode.BACKUP_FAILED, '备份写入失败', e));
  }
}
...
export async function getBackups(): Promise<Result<BackupMeta[]>> {
  try {
    ...
    return err(new AppError(ErrorCode.DB_READ_FAILED, '读取备份列表失败', e));
  }
}
...
export async function getLatestBackup(docId: string): Promise<Result<BackupData | null>> {
  try {
    ...
    return err(new AppError(ErrorCode.DB_READ_FAILED, '读取备份失败', e));
  }
}
...
export function exportAllToJson(docs: BackupData[]): Result<string> {
  try {
    ...
    return err(new AppError(ErrorCode.EXPORT_FAILED, '导出 JSON 失败', e));
  }
}
...
export function importFromJson(json: string): Result<BackupData[]> {
  try {
    ...
    if (!Array.isArray(parsed)) {
      return err(new AppError(ErrorCode.IMPORT_FAILED, 'JSON 格式不正确：需要文档数组'));
    }
    return ok(
    ...
    return err(
      new AppError(ErrorCode.IMPORT_FAILED, '导入 JSON 失败: ' + (e instanceof Error ? e.message : '格式错误'), e)
    );
  }
}
