import type { DocumentData, DocumentMeta, AutosavePayload, AutosaveResponse } from '../engine/types';
...
function writeDrafts(drafts: Record<string, DocumentData>): Result<void> {
  try {
    ...
    return err(new AppError(ErrorCode.DB_WRITE_FAILED, '写入文档失败', e));
  }
}
...
function writeIndex(index: DocumentMeta[]): Result<void> {
  try {
    ...
    return err(new AppError(ErrorCode.DB_WRITE_FAILED, '写入索引失败', e));
  }
}
...
export function createDocument(title?: string): Result<DocumentData> {
  try {
    ...
    return err(new AppError(ErrorCode.DB_WRITE_FAILED, '创建文档失败', e));
  }
}
...
export function updateDocument(id: string, data: Partial<DocumentData>): Result<DocumentData | null> {
  try {
    ...
    return err(new AppError(ErrorCode.DB_WRITE_FAILED, '更新文档失败', e));
  }
}
...
export function deleteDocument(id: string): Result<void> {
  try {
    ...
    return err(new AppError(ErrorCode.DB_DELETE_FAILED, '删除文档失败', e));
  }
}
