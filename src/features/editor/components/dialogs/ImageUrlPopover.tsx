  ...
  return (
    <div className="rte-dialog-backdrop" onClick={onClose}>
      <div className="rte-dialog" onClick={(e) => e.stopPropagation()}>
        <h3 className="text-lg font-semibold mb-4">{t('editor.insertImage')}</h3>
        <form onSubmit={handleSubmit} className="flex flex-col gap-2">
          <label className="text-sm font-medium text-deep-text/70 block mb-1">{t('editor.imageUrl')}</label>
          <input
            type="url"
            className="rte-input"
            value={src}
            ...
            autoFocus
          />
          <label className="text-sm font-medium text-deep-text/70 block mb-1">{t('editor.altText')}</label>
          <input
            type="text"
            className="rte-input"
            value={alt}
