    ...
    if (error) {
      if (retries < MAX_RETRIES) {
        ...
        if (retries > 0) {
          ...
          return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] border border-surface-3 rounded-lg bg-card-bg gap-3 px-4">
              <span className="loading loading-spinner loading-lg text-primary" />
              <span className="text-deep-text/70 text-sm">
        ...
        return (
          <div className="flex flex-col items-center justify-center min-h-[60vh] border border-surface-3 rounded-lg bg-card-bg gap-3 px-4">
            <span className="loading loading-spinner loading-lg text-primary" />
            <span className="text-deep-text/70 text-sm">编辑器加载异常，正在重试……</span>
          </div>
        );
      }
      ...
      // 3 次重试全部失败，显示降级提示
      return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] border border-surface-3 rounded-lg bg-card-bg gap-4 px-6">
          <svg
          ...
          <p className="text-deep-text/60 text-sm text-center max-w-md">
            已尝试 {MAX_RETRIES} 次加载，仍无法启动编辑器。可能的原因：
          </p>
          <ul className="text-deep-text/50 text-xs list-disc list-inside text-left">
            <li>开发服务器预构建缓存过期</li>
            <li>网络连接问题</li>
            <li>浏览器缓存冲突</li>
          </ul>
