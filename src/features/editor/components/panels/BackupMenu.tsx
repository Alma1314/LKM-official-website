...
import { listDocuments, createDocument, getDocument, autosave } from '../../stores/document-api';
import { exportAllToJson, importFromJson, getBackups, restoreFromBackup, saveBackup } from '../../stores/backup-store';
import type { BackupData, BackupMeta } from '../../stores/backup-store';
...
export default function BackupMenu() {
  ...
  const handleExport = useCallback(() => {
    ...
    const jsonResult = exportAllToJson(fullDocs as unknown as BackupData[]);
    if (!jsonResult.ok) {
      alert('导出失败: ' + jsonResult.error.message);
      return;
    }
  ...
  const handleShowBackups = useCallback(async () => {
    ...
    if (result.ok) {
      ...
      console.warn('[BackupMenu] 读取备份失败:', result.error.message);
    }
    setShowBackups(true);
  }, []);
  ...
  const handleRestore = useCallback(async (docId: string, title: string) => {
    ...
    const backupResult = await restoreFromBackup(docId);
    if (!backupResult.ok) {
      alert('读取备份失败: ' + backupResult.error.message);
      return;
    }
