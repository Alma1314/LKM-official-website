/* ----------------------------------------------------------------
   一帧的处理流程：推进时钟、松弛电场、生长通道、
   依次绘制天空 -> 地形 -> 通道、泛光、合成，然后处理
   异步回读（选择、父节点、标志、残差）。
   ---------------------------------------------------------------- */
...
export function frame(tms) {
  ...
  if (
    ...
    const wx = Math.ceil(GW / 8),
      wy = Math.ceil(GH / 8);
    /* 每次生长步之间都必须重新松弛电场：DBM 的分支抑制本质上就
       是静电屏蔽，而过期的 φ 会把先导压成一团 DLA 丛。
       偶数次扫描能保证 φ 落在 phiA 中。 */
    const jac = (n) => {
      p.setPipeline(plJac);
      for (let i = 0; i < n; i++) {
        p.setBindGroup(0, i % 2 === 0 ? bgJacAB : bgJacBA);
        p.dispatchWorkgroups(wx, wy, 1);
      }
    };
    jac(rt.jacIters);
    /* 每 10 帧进行一次残差探针：异步进行，绝不阻塞流水线 */
    if (rt.needFork) {
      rt.needFork = false;
  ...
  rp.draw(GW * 6); // 每个 x 列一个四边形，6 个顶点 = 2 个三角形
  ...
  for (let i = 0; i < BLOOM_LEVELS; i++)
    post(bloomView[i], "clear", i === 0 ? plDownFirst : plDown, bgDown[i]);
    ...
    resStage
      ...
      .then(() => {
        ...
        rt.residNow = new Float32Array(
          new Uint32Array([b & 0x80000000 ? b ^ 0x80000000 : ~b >>> 0]).buffer,
        )[0];
