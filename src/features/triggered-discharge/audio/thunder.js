/* ----------------------------------------------------------------
   雷声。每一个被采样的通道段都是一个独立的声源，
   按其到摄像机的真实距离延迟到达时间，并按传播距离进行低通滤波。
   隆隆声的时长就是闪电本身的几何形状。
   ---------------------------------------------------------------- */
...
export function thunder(peakKA, segs) {
  ...
  /* 爆裂声：只取最近的通道段 —— 尖锐的 N 波只沿最短路径存活；
     空气每经过一个额外的公里都会吸收高频成分 */
  const near = segs[0];
  ...
  /* 隆隆声：每一个被采样的通道段都按自己的时钟到达。
     ...
     更高的通道段承载着回击已衰减的电流：声音更轻，
     而且它们额外的传播路径会将其低通滤波，融入巨大的轰鸣之中。 */
  ...
  for (const sg of segs) {
    ...
    const hi = Math.exp(-sg.frac * 1.1); // MTLE：电流随高度衰减
    shot(
  ...
  for (let i = 0; i < steps; i++)
    curve[i] = 0.4 * v0 * Math.exp(-i / (steps * 0.45)) * (0.4 + Math.random());
