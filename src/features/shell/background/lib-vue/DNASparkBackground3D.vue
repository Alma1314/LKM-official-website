...
import { useCanvasViewport } from '../vue/useCanvasViewport';
...
const _connectionColor = props.connectionColor || (mode.value === 'dark' ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.08)');
...
const performance = backgroundPerformance.value;
...
const actualBasePairCount = Math.min(props.basePairCount, qualityConfig.basePairs);
const actualParticleCount = Math.min(props.particleCount, qualityConfig.particleCount);
const actualBackboneSegments =
  quality === 'low'
    ? Math.min(props.backboneSegments, 40)
    : quality === 'medium'
      ? Math.min(props.backboneSegments, 64)
      : props.backboneSegments;
const actualBaseSphereSegments =
  quality === 'low'
    ? Math.min(props.baseSphereSegments, 8)
    : quality === 'medium'
      ? Math.min(props.baseSphereSegments, 12)
      : props.baseSphereSegments;
...
const timeRef = ref(0);
const waveOffsetRef = ref(0);
const particlesRef = ref<ParticleData[]>([]);
const basePairsRef = ref<BasePairData[]>([]);
const backboneRef = ref<BackboneData | null>(null);
const raycasterRef = ref(new THREE.Raycaster());
const clickRipplesRef = ref<ClickRippleData[]>([]);
const nucleotideGroupsRef = ref<NucleotideGroupData[]>([]);
...
const initMolecularStructures3D = (scene: THREE.Scene) => {
  ...
  const halfH = Math.tan(fovRad / 2) * props.cameraOrbitRadius * 0.7;
  const halfW = halfH * aspect;

  nucleotideGroupsRef.value = [];
  structs.forEach((s, idx) => {
    ...
    scene.add(group);
    nucleotideGroupsRef.value.push({ group, base: s.base, side: s.side });
  });
};

const initDNA3D = (scene: THREE.Scene) => {
  const sequence = generateDNASequence(actualBasePairCount);
  const h = props.helixHeight * 2;
  const turns = 8;
  const curve1 = new THREE.CatmullRomCurve3(
    Array.from({ length: actualBackboneSegments }, (_, i) => {
      ...
      return new THREE.Vector3(Math.cos(t) * props.helixRadius, y, Math.sin(t) * props.helixRadius);
    })
  );
  ...
  const backboneGeo1 = new THREE.TubeGeometry(curve1, actualBackboneSegments, props.backboneRadius, 8, false);
  const backboneGeo2 = new THREE.TubeGeometry(curve2, actualBackboneSegments, props.backboneRadius, 8, false);
  ...
  backboneRef.value = { mesh1: backboneMesh1, mesh2: backboneMesh2 };
  ...
  for (let i = 0; i < seqLen; i++) {
    ...
    const pos1 = new THREE.Vector3(Math.cos(angle) * props.helixRadius, y, Math.sin(angle) * props.helixRadius);
    const pos2 = new THREE.Vector3(
      Math.cos(angle + Math.PI) * props.helixRadius,
      y,
      Math.sin(angle + Math.PI) * props.helixRadius
    );

    const baseGeometry = new THREE.SphereGeometry(
      props.baseSphereRadius,
...
const initParticles3D = (scene: THREE.Scene) => {
  const particleGeometry = new THREE.SphereGeometry(props.particleSize, 8, 8);
  ...
  for (let i = 0; i < actualParticleCount; i++) {
    ...
    scene.add(particle);
    const speed = Math.random() * (props.particleSpeedMax - props.particleSpeedMin) + props.particleSpeedMin;
...
const updateDNA3D = (time: number, waveOffset: number) => {
  if (backboneRef.value) {
    backboneRef.value.mesh1.rotation.y = time * props.dnaRotationSpeed;
    backboneRef.value.mesh2.rotation.y = time * props.dnaRotationSpeed;
  }
  basePairsRef.value.forEach((basePair, index) => {
    const waveAmplitude = Math.sin(waveOffset + index * 0.2) * 2;
    const rotationOffset = time * props.dnaRotationSpeed + index * 0.2;
...
const updateParticles3D = (frameScale: number) => {
  if (!cameraRef.value || width <= 0 || height <= 0) return;
  const mouse3D = new THREE.Vector3();
  const mx = mouseRef.value.x !== null ? (mouseRef.value.x / width) * 2 - 1 : 0;
  const my = mouseRef.value.y !== null ? -(mouseRef.value.y / height) * 2 + 1 : 0;
  mouse3D.x = mx * 100;
  mouse3D.y = my * 100;
  mouse3D.z = cameraRef.value.position.z * 0.1;
  ...
  particlesRef.value.forEach((particle) => {
    ...
    const orbital = new THREE.Vector3(
      Math.cos(particle.angle) * props.particleOrbitalAmplitude,
      Math.sin(particle.angle * 0.7) * props.particleOrbitalAmplitude,
      Math.sin(particle.angle) * props.particleOrbitalAmplitude
    );
...
const updateClickRipples3D = (frameScale: number) => {
  ...
  clickRipplesRef.value = clickRipplesRef.value.filter((ripple) => {
    ripple.scale += props.rippleGrowthRate * frameScale;
    ripple.opacity -= props.rippleFadeRate * frameScale;
    ...
    if (ripple.opacity <= 0) {
      sceneRef.value!.remove(ripple.mesh);
...
const updateGlowEffects = () => {
  ...
  const mx = mouseRef.value.x !== null ? (mouseRef.value.x / width) * 2 - 1 : 0;
  const my = mouseRef.value.y !== null ? -(mouseRef.value.y / height) * 2 + 1 : 0;
  raycasterRef.value.setFromCamera(new THREE.Vector2(mx, my), cameraRef.value);
  basePairsRef.value.forEach((basePair) => {
    const distance1 = raycasterRef.value.ray.distanceToPoint(basePair.mesh1.position);
    const distance2 = raycasterRef.value.ray.distanceToPoint(basePair.mesh2.position);
    const minDistance = Math.min(distance1, distance2);
    if (minDistance < props.glowRadius) {
      basePair.glowIntensity = Math.max(0, 1 - minDistance / props.glowRadius);
    } else {
      basePair.glowIntensity *= 0.95;
    }
    const emissiveIntensity = basePair.glowIntensity * props.glowIntensityMultiplier * qualityConfig.glow;
...
onMounted(() => {
  ...
  const camera = new THREE.PerspectiveCamera(75, (size.value.width || 1) / (size.value.height || 1), 0.1, 1000);
  camera.position.set(0, 0, props.cameraOrbitRadius);
  ...
  renderer.setSize(size.value.width || 1, size.value.height || 1);
  renderer.setPixelRatio(backgroundPerformance.value.dpr);
  ...
  const stopWatchSize = watch(size, resizeRenderer);
  ...
  const onPointerDown = (e: PointerEvent) => {
    ...
    const w = size.value.width || 1;
    const h = size.value.height || 1;
    if (!sceneRef.value || !cameraRef.value || w <= 0 || h <= 0) return;
    const rmx = (rx / w) * 2 - 1;
    const rmy = -(ry / h) * 2 + 1;
    raycasterRef.value.setFromCamera(new THREE.Vector2(rmx, rmy), cameraRef.value);
    ...
    if (intersects.length > 0) {
      ...
      rippleMesh.lookAt(cameraRef.value.position);
    }
    sceneRef.value.add(rippleMesh);
    clickRipplesRef.value.push({ mesh: rippleMesh, scale: 1, opacity: 0.8 });
  };
  renderer.domElement.addEventListener('pointerdown', onPointerDown);

  const disposeScene = () => {
    stopWatchSize();
    if (onPointerDown && renderer.domElement) {
      renderer.domElement.removeEventListener('pointerdown', onPointerDown);
    }
    for (const ripple of clickRipplesRef.value) {
      scene.remove(ripple.mesh);
      ripple.mesh.geometry.dispose();
      (ripple.mesh.material as THREE.Material).dispose();
    }
    clickRipplesRef.value = [];
    ...
    backboneRef.value = null;
    basePairsRef.value = [];
    particlesRef.value = [];
    nucleotideGroupsRef.value = [];
    clickRipplesRef.value = [];
    timeRef.value = 0;
    waveOffsetRef.value = 0;
  };
  ...
  const loop = (now: number) => {
    rafIdRef.value = requestAnimationFrame(loop);
    const interval = backgroundPerformance.value.frameInterval;
    ...
    timeRef.value += deltaSeconds * qualityConfig.motion;
    waveOffsetRef.value += deltaSeconds * 2.5 * qualityConfig.motion;
    const time = timeRef.value;
    const waveOffset = waveOffsetRef.value;

    if (cameraRef.value) {
      cameraRef.value.position.x = Math.cos(time * props.cameraOrbitSpeed) * props.cameraOrbitRadius;
      cameraRef.value.position.z = Math.sin(time * props.cameraOrbitSpeed) * props.cameraOrbitRadius;
    ...
    if (cameraRef.value) {
      ...
      const aspectC = width > 0 && height > 0 ? width / height : 1;
      const fovRad = (75 * Math.PI) / 180;
      const dist = 120;
      const halfH = Math.tan(fovRad / 2) * dist;
      const halfW = halfH * aspectC;

      nucleotideGroupsRef.value.forEach((g, i) => {
...
<template>
  <div
    ...
    style="position: absolute; inset: 0; z-index: 0; pointer-events: auto; background: transparent"
  />
</template>
