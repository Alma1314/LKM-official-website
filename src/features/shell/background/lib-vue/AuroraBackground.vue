...
function draw(frame: BackgroundFrame) {
  ...
  if (quality !== 'low') {
    for (const r of frame.ripples) ripples.push({ x: r.x, y: r.y, radius: 0, opacity: 1, growing: true });
    if (ripples.length > MAX) ripples.splice(0, ripples.length - MAX);
  }
  ctx.clearRect(0, 0, width, height);
  for (let l = 0; l < vl; l++) {
    ctx.beginPath();
    const wh = props.baseWaveHeight! + l * props.waveSpacing!;
    const wo = time * ws + l * 50;
    ctx.strokeStyle = theme.colors[l % theme.colors.length];
    ctx.globalAlpha = theme.alpha[l % theme.alpha.length];
    ctx.lineWidth = props.lineWidthBase! + l * 1.5;
    for (let x = 0; x <= width; x += ss) {
      ...
      if (mouse.x !== null && mouse.y !== null) {
        ...
        fy = y - (dist < props.mouseRadius! ? ((props.mouseRadius! - dist) / props.mouseRadius!) * mstr : 0);
      }
      ctx.lineTo(x, fy);
    }
    ctx.stroke();
  ...
  if (quality !== 'low') {
    for (let i = ripples.length - 1; i >= 0; i--) {
      const r = ripples[i]!;
      if (r.growing) {
        r.radius += props.rippleGrowthRate! * delta * 60 * qs;
