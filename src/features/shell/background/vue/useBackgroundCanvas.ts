...
export function getBackgroundPerformance(maxDpr = Number.POSITIVE_INFINITY): BackgroundPerformance {
  ...
  const s = {
    high: { dpr: 1.5, fi: 1000 / 60 },
    medium: { dpr: 1.25, fi: 1000 / 45 },
    low: { dpr: 1, fi: isMobile ? 1000 / 20 : 1000 / 30 },
  }[quality];
  return {
    quality,
    dpr: Math.min(window.devicePixelRatio || 1, quality === 'low' && isMobile ? 1 : s.dpr, maxDpr),
    frameInterval: s.fi,
    reducedMotion,
  };
}
...
export function useBackgroundCanvas({
  ...
  function setup() {
    ...
    const useMouse = !!interactions.mouse,
      useClick = !!interactions.click,
      useKeys = !!interactions.keys;
