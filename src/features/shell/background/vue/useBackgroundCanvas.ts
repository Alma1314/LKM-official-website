...
export function getBackgroundPerformance(maxDpr = Number.POSITIVE_INFINITY): BackgroundPerformance {
  ...
  const s = { high: { dpr: 1.5, fi: 1000 / 60 }, medium: { dpr: 1.25, fi: 1000 / 45 }, low: { dpr: 1, fi: isMobile ? 1000 / 20 : 1000 / 30 } }[quality];
  return { quality, dpr: Math.min(window.devicePixelRatio || 1, quality === 'low' && isMobile ? 1 : s.dpr, maxDpr), frameInterval: s.fi, reducedMotion };
}

export interface BackgroundFrame {
  ctx: CanvasRenderingContext2D; width: number; height: number; dpr: number; performance: BackgroundPerformance;
  mouse: BackgroundMouse; ripples: Ripple[]; keys: { shift: boolean }; time: number; delta: number; sinceLastResize: number;
}
...
export interface UseBackgroundCanvasOptions {
  ...
  interactions?: BackgroundInteractions; maxDpr?: number;
}
...
export function useBackgroundCanvas({ draw, init, interactions = {}, maxDpr = DEFAULT_MAX_DPR }: UseBackgroundCanvasOptions) {
  ...
  let frameRef: BackgroundFrame | null = null, rafId: number | null = null, startTime = 0, pausedElapsed = 0;
  let lastActualDrawTime = 0, lastDrawTime = 0, paused = false, initCleanup: (() => void) | void, cleanup: (() => void) | null = null;
  let drawFn = draw, initFn = init;

  function setup() {
    const canvas = canvasRef.value; if (!canvas) return;
    const ctx = canvas.getContext('2d'); if (!ctx) return;
    drawFn = draw; initFn = init;
    const bp = getBackgroundPerformance(maxDpr);
    let sinceLastResize = 0;
    const frame: BackgroundFrame = { ctx, width: 0, height: 0, dpr: bp.dpr, performance: bp, mouse: { x: null, y: null, isDown: false }, ripples: [], keys: { shift: false }, time: 0, delta: 0, sinceLastResize: 0 };
    frameRef = frame; let isVisible = !document.hidden, isInViewport = true, pausedStart: number | null = null;
    startTime = performance.now(); lastActualDrawTime = startTime; pausedElapsed = 0; paused = false;

    const resize = () => {
      sinceLastResize = 0; const w = window.innerWidth, h = window.innerHeight;
      frame.performance = getBackgroundPerformance(maxDpr); frame.dpr = frame.performance.dpr;
      canvas.width = Math.max(1, Math.round(w * frame.dpr)); canvas.height = Math.max(1, Math.round(h * frame.dpr));
      frame.width = w; frame.height = h; ctx.setTransform(frame.dpr, 0, 0, frame.dpr, 0, 0);
    };
    resize(); initCleanup = initFn?.(canvas, frame);
    const onResize = () => resize(); window.addEventListener('resize', onResize);
    const reducedQuery = window.matchMedia('(prefers-reduced-motion: reduce)');
    const onReducedMotionChange = () => {
      const wasReduced = frame.performance.reducedMotion; resize();
      const now = performance.now(); lastActualDrawTime = now; lastDrawTime = now - frame.performance.frameInterval;
      if (!wasReduced && frame.performance.reducedMotion) frame.ripples = [];
    };
    reducedQuery.addEventListener('change', onReducedMotionChange);
    const updatePause = () => {
      const shouldPause = !(isVisible && isInViewport); if (shouldPause === paused) return;
      const now = performance.now(); paused = shouldPause;
      if (shouldPause) pausedStart = now; else if (pausedStart !== null) { pausedElapsed += now - pausedStart; pausedStart = null; }
    ...
    const useMouse = !!interactions.mouse, useClick = !!interactions.click, useKeys = !!interactions.keys;
    const getCanvasPos = (e: PointerEvent) => ({ x: e.clientX, y: e.clientY });
    const onPointerMove = (e: PointerEvent) => { const p = getCanvasPos(e); frame.mouse.x = p.x; frame.mouse.y = p.y; };
    const onPointerDown = (e: PointerEvent) => {
      const p = getCanvasPos(e); frame.mouse.x = p.x; frame.mouse.y = p.y; frame.mouse.isDown = true;
      if (useClick && !frame.performance.reducedMotion) frame.ripples.push({ x: p.x, y: p.y, startTime: frame.time });
    };
    ...
    const loop = (now: number) => {
      ...
      lastDrawTime += intervalsElapsed * interval; if (now - lastDrawTime > interval) lastDrawTime = now;
      if (sinceLastResize < 3) sinceLastResize++; frame.sinceLastResize = sinceLastResize;
      frame.time = (now - startTime - pausedElapsed) / 1000;
      frame.delta = Math.min((now - lastActualDrawTime) / 1000, 0.1);
      const ripplesBatch = frame.ripples; frame.ripples = [];
      drawFn({ ...frame, ripples: ripplesBatch }); lastActualDrawTime = now;
    };
    rafId = requestAnimationFrame(loop);
    ...
    cleanup = () => {
      ...
      if (rafId !== null) cancelAnimationFrame(rafId); window.removeEventListener('resize', onResize);
      reducedQuery.removeEventListener('change', onReducedMotionChange); io.disconnect(); document.removeEventListener('visibilitychange', onVisibility);
      ...
      initCleanup?.(); initCleanup = undefined; frameRef = null;
    };
  }
