...
onMounted(() => {
  ...
  if (stored && stored !== DEFAULT_BACKGROUND) {
    const se = BACKGROUNDS.find((b) => b.id === stored);
    if (se?.preload) se.load();
  }
...
const activeEntry = computed(
  () => BACKGROUNDS.find((b) => b.id === currentBg.value) || BACKGROUNDS.find((b) => b.id === DEFAULT_BACKGROUND)!
);
...
function switchBg(id: BackgroundId) {
  currentBg.value = id;
...
function onDocMouseDown(e: MouseEvent) {
  ...
  if (panelRef.value && !panelRef.value.contains(t) && btnRef.value && !btnRef.value.contains(t))
    panelOpen.value = false;
}
