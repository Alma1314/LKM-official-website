...
const currentBg = ref<BackgroundId>(DEFAULT_BACKGROUND); const isDark = ref(false);
const panelOpen = ref(false); const panelRef = ref<HTMLElement | null>(null); const btnRef = ref<HTMLElement | null>(null);
...
onMounted(() => {
  ...
  if (stored && BACKGROUNDS.some(b => b.id === stored) && stored !== DEFAULT_BACKGROUND) currentBg.value = stored as BackgroundId;
  const defaultEntry = BACKGROUNDS.find(b => b.id === DEFAULT_BACKGROUND)!;
  if (defaultEntry.preload) defaultEntry.load();
  if (stored && stored !== DEFAULT_BACKGROUND) { const se = BACKGROUNDS.find(b => b.id === stored); if (se?.preload) se.load(); }
  portalTarget.value = document.querySelector('[data-hero-section]');
});

const visibleBackgrounds = computed(() => BACKGROUNDS.filter(b => b.theme === 'both' || b.theme === (isDark.value ? 'dark' : 'light')));
...
function onDocMouseDown(e: MouseEvent) {
  ...
  if (panelRef.value && !panelRef.value.contains(t) && btnRef.value && !btnRef.value.contains(t)) panelOpen.value = false;
}
function onDocKeydown(e: KeyboardEvent) { if (e.key === 'Escape') panelOpen.value = false; }
watch(panelOpen, (val) => {
  if (val) { document.addEventListener('mousedown', onDocMouseDown); document.addEventListener('keydown', onDocKeydown); }
  else { document.removeEventListener('mousedown', onDocMouseDown); document.removeEventListener('keydown', onDocKeydown); }
});
onBeforeUnmount(() => { document.removeEventListener('mousedown', onDocMouseDown); document.removeEventListener('keydown', onDocKeydown); });
</script>
      ...
      <div style="pointer-events: auto">
        ...
        <div v-if="panelOpen" ref="panelRef"
          ...
          <button v-for="bg in visibleBackgrounds" :key="bg.id"
            ...
            <span class="text-lg">{{ bg.icon }}</span><span>{{ bg.name }}</span>
          </button>
        </div>
      </div>
    </Teleport>
