  ...
  <div>
    ...
    >
      <div
        v-for="i in 4"
        :key="i"
        class="bg-card-bg border border-surface-3 rounded-xl p-5 animate-pulse h-24"
      />
    </div>

    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <div class="bg-card-bg border border-surface-3 rounded-xl p-5">
        <div class="text-2xl font-bold text-deep-text">
          {{ stats?.user_count ?? 0 }}
        </div>
      ...
      <div class="bg-card-bg border border-surface-3 rounded-xl p-5">
        <div class="text-2xl font-bold text-yellow-500">
          {{ stats?.file_pending_count ?? 0 }}
        </div>
        <div class="text-sm text-text-muted">
          {{ t("admin.stats.pendingFiles") }}
        </div>
      </div>
    </div>
  </div>
