<script lang="ts">
  ...
  let notifications: MockNotification[] = $state(mockNotifications);
  ...
  {#if isOpen}
    ...
    >
      <div class="flex items-center justify-between px-3 py-2 border-b border-surface-3 mb-1">
        ...
        {#if unreadCount > 0}
          <button class="text-xs text-primary hover:underline" onclick={markAllAsRead}> 全部已读 </button>
        {/if}
      </div>

      {#if notifications.length === 0}
        <div class="px-3 py-6 text-center text-sm text-text-muted">暂无通知</div>
      {:else}
        {#each notifications as n}
          ...
          >
            <span
              class="shrink-0 mt-0.5 w-8 h-8 rounded-full flex items-center justify-center {n.isRead
            ...
            {#if !n.isRead}
              <span class="shrink-0 w-2 h-2 rounded-full bg-primary mt-1.5"></span>
            {/if}
          </button>
        {/each}
      {/if}
