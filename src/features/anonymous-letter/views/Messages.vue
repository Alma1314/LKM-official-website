    ...
    <div class="msg-layout">
      ...
      <section class="chat glass" v-if="active">
        ...
        <div class="chat-input" v-if="!active.blocked">
          <textarea v-model="text" placeholder="匿名回复…" @keyup.enter.exact="send" rows="1" :class="{ 'send-ripple': rippling }" @animationend="rippling = false"></textarea>
...
.bubble { max-width: 72%; padding: 10px 14px; border-radius: 16px; font-size: calc(14px * var(--font-scale)); line-height: 1.6; word-break: break-word; animation: floatUp .3s both; position: relative; }
...
@media (max-width: 768px) { .chat-empty { grid-column: 1; } }
</style>
