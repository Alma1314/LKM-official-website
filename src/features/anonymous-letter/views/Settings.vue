<template>
  <div class="settings">
    ...
    <p class="foot-note">数据保存在服务器（匿名 + 可选登录），投稿经审核后展示。</p>
...
const { state, isNight, setTheme, setFontScale, toggleMuted, setAccent, toggleLowPerf, toggleHighContrast, setRateLimit, lowPerf, highContrast } = useApp()
...
.theme-opt { border: 1px solid var(--card-border); background: rgba(255,255,255,0.4); color: var(--text-sub); border-radius: 999px; padding: 6px 16px; font-size: 13px; cursor: pointer; transition: all .2s; }
...
.switch { width: 50px; height: 28px; border-radius: 999px; border: 1px solid var(--card-border); background: transparent; position: relative; cursor: pointer; transition: background .3s; flex-shrink: 0; }
.switch.on { background: var(--grad-soft); border-color: var(--blue); }
.knob { position: absolute; top: 3px; left: 3px; width: 22px; height: 22px; border-radius: 50%; background: #fff; transition: transform .3s; box-shadow: 0 2px 6px rgba(0,0,0,0.2); }
.switch.on .knob { transform: translateX(22px); }
.mini { border: 1px solid var(--card-border); background: rgba(255,255,255,0.4); color: var(--text-main); border-radius: 999px; padding: 6px 16px; font-size: 13px; cursor: pointer; }
