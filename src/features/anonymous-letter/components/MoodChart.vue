  ...
  <div class="mood-chart glass">
    ...
    <div v-if="hasData" class="mc-body">
      <svg :viewBox="`0 0 320 ${h}`" class="mc-svg">
        <!-- 网格线 -->
        <line v-for="g in gridY" :key="'g'+g" :x1="40" :x2="310" :y1="g" :y2="g" stroke="var(--card-border)" stroke-width="1" />
        <!-- 柱子 -->
        <g v-for="(d, i) in bars" :key="d.mood">
          <rect
            :x="50 + i * 38" :y="d.y" :width="26" :height="h - 30 - d.y"
            rx="6" :fill="colorOf(d.mood)" class="mc-bar"
          />
          <text :x="63 + i * 38" :y="h - 12" text-anchor="middle" font-size="9" fill="var(--text-sub)">{{ d.mood }}</text>
          <text :x="63 + i * 38" :y="d.y - 4" text-anchor="middle" font-size="9" fill="var(--text-main)">{{ d.count }}</text>
        </g>
        <line x1="40" :y1="h - 30" x2="310" :y2="h - 30" stroke="var(--text-sub)" stroke-width="1.5" />
      </svg>
      <p class="mc-tip">本月共记录 <b>{{ total }}</b> 次情绪，最常见：<b :style="{ color: topColor }">{{ topMood }}</b></p>
    </div>
...
<style scoped>
.mood-chart { padding: 16px 18px; border-radius: var(--radius); }
.mc-head { display: flex; justify-content: space-between; align-items: center; font-weight: 700; margin-bottom: 10px; }
.mc-select { border-radius: var(--radius-pill); border: 1px solid var(--card-border); background: rgba(255,255,255,0.4); color: var(--text-main); padding: 4px 10px; font-size: 12px; }
.mc-svg { width: 100%; height: auto; }
.mc-bar { transition: height .4s; }
.mc-tip { font-size: 12px; color: var(--text-sub); text-align: center; margin: 8px 0 0; }
</style>
