...
function stop() {
  try { node && node.stop(); ctx && ctx.close() } catch (e) {}
  node = null; ctx = null
}
function toggle() {
  playing.value = !playing.value
  store.saveSettings({ audioOn: playing.value })
  if (playing.value) start(); else stop()
}
function onVol(e) {
...
.audio-player {
  ...
  background: rgba(255,255,255,0.4); border: 1px solid var(--card-border);
}
.audio-btn { border: none; background: none; cursor: pointer; font-size: 18px; }
.audio-label { font-size: 12px; color: var(--text-sub); }
.audio-vol { width: 70px; accent-color: var(--accent); }
</style>
