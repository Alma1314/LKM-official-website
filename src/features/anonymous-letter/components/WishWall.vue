...
function colorOf(id) { return COLORS[Math.abs(hash(id)) % COLORS.length] }
...
function make() {
  ...
  text.value = ''; openMake.value = false
  ElMessage({ message: '愿望已点亮 🌟', type: 'success', customClass: 'th-toast' })
}

function openEdit(w) { editId.value = w.id; editText.value = w.text; openEditBox.value = true }

function saveEdit() {
  if (!editText.value.trim()) return
  const list = getWishes()
  const i = list.findIndex(x => x.id === editId.value)
  if (i > -1) { list[i].text = editText.value.trim(); saveWishes(list) }
  wishes.value = list
...
function remove(w) {
  ElMessageBox.confirm('确定删除这个愿望吗？', '确认删除', { confirmButtonText: '删除', cancelButtonText: '取消', type: 'warning' }).then(() => {
...
<style scoped>
.wish-wall { display: flex; flex-direction: column; gap: 12px; }
.ww-head { display: flex; justify-content: space-between; align-items: center; }
.ww-head h3 { margin: 0; font-size: 18px; }
.ww-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 12px; }
.wish-note { padding: 14px; border-radius: var(--radius-sm); border-left: 4px solid var(--wc); min-height: 90px; display: flex; flex-direction: column; justify-content: space-between; position: relative; }
.wish-text { margin: 0; font-size: calc(13px * var(--font-scale)); line-height: 1.6; }
.wish-foot { display: flex; justify-content: space-between; align-items: center; font-size: 11px; color: var(--text-sub); margin-top: 6px; }
.wish-light { cursor: pointer; transition: transform .2s; }
.wish-light:active { transform: scale(1.3); }
.wish-acts { display: flex; gap: 6px; margin-top: 8px; }
.wish-acts .mini { border: 1px solid var(--card-border); background: rgba(255,255,255,0.4); color: var(--text-main); border-radius: 999px; padding: 2px 10px; font-size: 11px; cursor: pointer; transition: all .2s; }
.wish-acts .mini:hover { border-color: var(--accent); }
.wish-acts .mini.danger:hover { border-color: var(--danger); color: var(--danger); }
.wish-empty { grid-column: 1/-1; text-align: center; color: var(--text-sub); padding: 24px; }
</style>
