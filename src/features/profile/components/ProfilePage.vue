<template>
  <div class="space-y-6">
    ...
    <!-- 个人资料 -->
    <div class="bg-card-bg border border-surface-3 rounded-2xl p-6">
      <h2 class="text-lg font-semibold text-deep-text mb-4">个人资料</h2>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-1 mt-1">
            <span v-for="i in user.interests" :key="i" class="text-xs px-2 py-0.5 rounded-full bg-surface-3 text-text-muted">{{ i }}</span>
          </div>
        </div>
    ...
    <!-- Tab 区域 -->
    <div class="bg-card-bg border border-surface-3 rounded-2xl overflow-hidden">
      ...
      <div class="p-6">
        ...
        <div v-if="activeTab === 'projects'" class="space-y-3">
          <div v-for="(proj, i) in userProjects" :key="i" class="flex items-start gap-3 border-b border-surface-3 pb-3 last:border-0 last:pb-0">
            ...
            <div class="flex-1 min-w-0">
              <a :href="`/projects/${proj.id}`" class="text-deep-text font-medium hover:text-primary transition-colors line-clamp-1">{{ proj.name }}</a>
        ...
        <div v-if="activeTab === 'columns'" class="space-y-3">
          <div v-if="!user.hasColumnAccess" class="text-center py-8">
            ...
            <a href="/register/onboarding" class="text-primary text-sm font-medium hover:underline">通过答题解锁专栏 →</a>
          </div>
...
const userPosts = computed(() =>
  mockPosts.filter((p) => p.authorId === props.user.id).slice(0, 10)
);
