...
describe('useAuthStore 单一状态源', () => {
  ...
  it('restoreAndValidate 校验失败时原子清除', async () => {
    ...
    vi.spyOn(authApi, 'getMe').mockResolvedValue(err(new AppError(ErrorCode.UNKNOWN_ERROR, 'x')));
    const s = useAuthStore();
    s.setTokens('t', 'r');
  ...
  it('loginPassword 失败保持 anonymous', async () => {
    ...
    vi.spyOn(authApi, 'loginPassword').mockResolvedValue(err(new AppError(ErrorCode.AUTH_ERROR, '账号或密码错误')));
    const s = useAuthStore();
