...
export function useAuthProvider(): AuthContextType {
  ...
  function applyLoginResult(success: {
    requires2FA?: boolean;
  ...
  async function login(
    ...
    if (method === "passkey") {
      ...
      return err(
        new AppError(
          ...
          t("messages.auth.passkeyViaLoginPage"),
        ),
      );
    }
    ...
    return err(
      new AppError(
        ErrorCode.AUTH_ERROR,
        t("messages.auth.unsupportedLoginMethod"),
      ),
    );
  }
  ...
  async function register(
    ...
    if (type === "local") {
      const result = await store.registerLocal(
        data.username,
        data.password || "",
      );
    ...
    return err(
      new AppError(
        ...
        t("messages.auth.unsupportedRegisterType"),
      ),
    );
  }
...
export function useAuth(): AuthContextType {
  ...
  if (!ctx)
    throw new Error(
      "useAuth must be used within a component that calls useAuthProvider()",
    );
  return ctx;
}
