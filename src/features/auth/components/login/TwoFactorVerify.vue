<template>
  <div class="space-y-4">
    <h2 class="text-xl font-semibold text-center mb-2">{{ t('auth.twoFactor.title') }}</h2>
    ...
    <div v-if="showRecovery" class="space-y-4">
      ...
      <button type="button" class="btn btn-ghost w-full btn-sm" @click="showRecovery = false">
    ...
    <form v-else @submit.prevent="handleTOTPSubmit" class="space-y-4">
      <p class="text-sm text-text-muted text-center">{{ t('auth.twoFactor.totpHint') }}</p>
      <AuthField
        id="totp-code"
...
const flow = useLoginFlow({ redirect: null, onSuccess: () => emit('success', t('auth.twoFactor.verifyPassed')) });
