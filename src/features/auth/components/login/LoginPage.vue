...
// 同步读取 ?redirect= 参数：若放到 onMounted 再传给 useLoginFlow，flow 的 redirect 在
// setup 阶段已固定为 '/'，导致成功后再解析时拿不到真实目标。组件为 client:only，可安全
// 在 setup 访问 window（沿用项目既有 typeof window 防御）。
