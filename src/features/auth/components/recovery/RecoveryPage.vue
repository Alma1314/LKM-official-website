  ...
  <div v-if="step === 'done'" class="relative min-h-[calc(100vh-12rem)] flex items-center justify-center px-4">
    ...
    <div class="relative w-full max-w-md text-center">
      <div class="rounded-2xl bg-card-bg shadow-2xl border border-surface-3 p-6 sm:p-8">
        ...
        <p class="text-sm text-text-muted">所有设备已强制下线，请使用新密码重新登录</p>
    ...
    <div class="relative w-full max-w-md">
      <div class="rounded-2xl bg-card-bg shadow-2xl border border-surface-3 p-6 sm:p-8">
        ...
        <div v-if="step === 'verify' && method === 'magic-link'" class="space-y-4 text-center">
          <p class="text-sm text-text-muted">重置链接已发送至 {{ account?.email }}（模拟）</p>
        ...
        <form v-if="step === 'verify' && method !== 'magic-link'" @submit.prevent="handleVerify" class="space-y-4">
          <p class="text-sm text-text-muted">验证码已发送（模拟码：000000）</p>
          <div>
        ...
        <form v-if="step === 'reset'" @submit.prevent="handleReset" class="space-y-4">
          <p class="text-sm text-text-muted">为 {{ account?.username }} 设置新密码</p>
          <div>
...
import { findAccount, VALIDATE_CODE } from '~/features/auth/data/demo-accounts';
