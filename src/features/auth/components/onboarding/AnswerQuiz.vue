  ...
  <div class="space-y-6">
      ...
      <div class="flex items-center justify-between mb-4">
        ...
        <div class="flex gap-1">
          <span
            ...
            :class="answers[i] === undefined ? 'bg-surface-3' : answers[i] === questions[i].answer ? 'bg-green-500' : 'bg-red-500'"
          ></span>
        </div>
      </div>
    ...
    <div v-else class="text-center space-y-4">
      <div class="w-20 h-20 mx-auto rounded-full flex items-center justify-center"
        ...
        <Icon :icon="passed ? 'material-symbols:check-circle-outline' : 'material-symbols:cancel-outline'"
          class="w-10 h-10" :class="passed ? 'text-green-500' : 'text-red-500'" />
      </div>
      <h3 class="text-lg font-semibold" :class="passed ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
        {{ passed ? '恭喜通过！' : '未通过' }}
      </h3>
      <p class="text-sm text-text-muted">
        正确 {{ correctCount }} / {{ questions.length }}（正确率 {{ Math.round(correctCount / questions.length * 100) }}%，{{ passed ? '≥60%' : '<60%' }}）
      </p>
...
const correctCount = computed(() => answers.value.reduce((sum, a, i) => sum + (a === questions.value[i]?.answer ? 1 : 0), 0));
